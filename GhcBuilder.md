...
This builder can be used to build Haskell programs with [Glasgow Haskell Compiler](http://haskell.org/ghc/) 

Russel Winder has created a Bazaar branch of a SCons Haskell tool based on this code.  It can be found at [https://code.launchpad.net/~russel/sconsaddons/haskell](https://code.launchpad.net/~russel/sconsaddons/haskell) 
...
```python
#!python 
env = Environment( tools = ["default", "ghc"], toolpath = ["."] )

env.HaskellProgram(
        "foo", ["bar.hs", "baz.hs"],
        HSPACKAGES = ["process", "mtl"], HSCFLAGS = ["-fglasgow-exts"]
        )
