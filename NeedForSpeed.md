...
SCons is essentially an orchestration tool: it figures out what needs to get done, but it calls external tools to actually perform nearly all of the building steps (the exceptions are actions written entirely in Python, but those tend to be rare and/or small).  In many cases, particularly small builds, the time SCons takes to figure out what it needs to do is dwarfed by the time spent once SCons calls out to the operating system to get `gcc` or `cl.exe` to actually compile a file. So the challenge is to find places where the "figuring out" is slow enough that it causes lost developer time.
 ...
 * The code is not doing enough work. This could occur if something is idling waiting for a condition to change - the main known place this could happen is jobs dispatch, if a thread is waiting for the work queue to be filled up again so it can pick another task to run.
 * The code is doing work is has to, but slowly. This might include Python language inefficiencies, too much recursion or otherwise deeply nested execution contexts, etc.  Certainly, running a scanner over all the dependencies in a project with thousands of files takes some time, and requires waiting for I/O. Profiling the code helps identify these areas.
