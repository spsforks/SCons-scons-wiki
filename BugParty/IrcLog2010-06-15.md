...
17:11:33  <[GregNoel](GregNoel)>     2647, Steven solved a nasty problem and checked in a fix, but should that fix be scheduled toward a release of 2.0.0.final.1, 2.0.1, or 2.1? I added a workaround to the issue that should work (Gary's suggestion of [SideEffect](SideEffect)() works perfectly), so if we deem that good, I vote for 2.1. 
